...
// Sum recurring income active in the given cycle (returns monthly total)
...
// Sum recurring expenses active in the given cycle (returns monthly total)
...
export async function getSavingsTarget() {
  // Use settings override first, then latest savings target record
...
export async function calcDailyAllowance(cycleStart, cycleEnd) {
  ...
  // Scale to actual cycle length vs 30-day month
...
// Get all variable transactions (non-daily-budget, non-distribution-child) in a date range
...
// Generate child transactions from a distribution record
...
// Cycle analysis for breakdown view
