...
const state = {
  ...
  viewingCycle: null,    // for breakdown nav
  pendingSync: 0,
};
...
async function saveEntry(overlay) {
  ...
  const txn = {
    ...
    amount: finalAmount,
    categoryId: state.entryCategory,
    note: state.entryNote.trim(),
...
async function renderTransactions() {
  ...
  if (query) {
    txns = txns.filter(t => t.note?.toLowerCase().includes(query));
  }
  ...
  viewContainer.innerHTML = `
    <div class="transactions-screen">
      <div class="screen-header">
        ...
        <button class="icon-btn" id="txn-add-btn" title="Add transaction">
          <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round">
      ...
      <div class="search-bar">
        ...
        <input type="text" id="txn-search" placeholder="Search transactions..."
               value="${state.txnSearchQuery}" style="flex:1;border:none;outline:none;font-size:15px;background:none">
      </div>
      ...
      <div id="txn-list-body">
        ${dates.length === 0 ? `
          <div class="empty-state">
            ...
            <div class="empty-text">Tap + to add your first transaction</div>
          </div>
        ` : dates.map(date => {
          ...
          return `
            <div class="day-group">
              <div class="day-header">
                <span>${dayLabel}</span>
                <button class="day-add-btn" data-date="${date}" title="Add on this day">+</button>
              </div>
              <div class="txn-list">
                ${dayTxns.map(t => {
                  ...
                  return `
                    <div class="txn-row" data-txn-id="${t.id}">
                      <div class="txn-cat-icon" style="background:${cat?.colour ?? '#ccc'}20">
                        ${cat?.icon ?? '📦'}
                      </div>
                      <div class="txn-info">
              ...
              <div class="day-total">
                <div class="day-total-amount">${dayTotal >= 0 ? '' : ''}${fmt(dayTotal)}</div>
              </div>
            </div>
          `;
        }).join('')}
  ...
  viewContainer.querySelector('#txn-search').oninput = e => {
    state.txnSearchQuery = e.target.value;
    renderTransactions();
  };

  delegate(viewContainer, 'click', '.txn-row', (e, el) => {
    const id = Number(el.dataset.txnId);
    showTxnMenu(id);
  });

  delegate(viewContainer, 'click', '.day-add-btn', (e, el) => {
...
async function showTxnMenu(id) {
  ...
  overlay.querySelector('#txn-del-btn').onclick = async () => {
    if (txn.distributionId) {
      showToast('Edit the parent distribution to modify this entry');
      overlay.remove();
      return;
    }
...
async function renderBreakdown() {
  ...
  const rows = [
    {
      icon: '💰', bg: '#e8f5e9', label: 'Income',
      amount: bd.totalIncome, amountClass: 'text-green',
      subs: [
        { label: 'Regular income', amount: bd.regularIncome },
        { label: 'Variable income', amount: bd.variableIncome },
      ],
    },
    {
      icon: '🛒', bg: '#ffebee', label: 'Expenses',
      amount: bd.totalExpenses, amountClass: 'text-red',
      subs: [
        { label: 'Recurring expenses', amount: bd.recurringExpenses },
        { label: 'Variable expenses', amount: bd.variableExpenses },
        { label: 'Big Expenses', amount: bd.distributionExpenses },
      ],
    },
    {
      icon: '💛', bg: '#fffde7', label: 'Savings',
      amount: bd.savings, amountClass: '',
      subs: [],
    },
    {
      icon: '📊', bg: '#e3f2fd', label: 'Budget left',
      amount: bd.budgetLeft, amountClass: bd.budgetLeft >= 0 ? 'text-green' : 'text-red',
      subs: [],
    },
  ];
  ...
  viewContainer.innerHTML = `
    <div class="breakdown-screen">
      ...
      <div style="padding:0 12px 16px">
        <button class="btn btn-primary btn-full" onclick="window.app.navigate('recurring')">
  ...
  viewContainer.querySelector('#cycle-prev').onclick = () => {
    ...
    state.viewingCycle = cycleForDate(isoDate(prev));
    renderBreakdown();
  };
...
async function renderRecurring() {
  ...
  viewContainer.innerHTML = `
    <div class="recurring-screen">
      ...
      <div class="recurring-list">
        ${items.length === 0 ? `
          <div class="empty-state">
            <div class="empty-icon">${tab === 'expenses' ? '💸' : '💰'}</div>
            <div class="empty-title">No active ${tab}</div>
            <div class="empty-text">Tap + to add one</div>
          </div>
        ` : items.map(r => `
        ...
        <button class="btn btn-primary btn-full" id="rec-add-btn" style="margin-top:8px">
  ...
  delegate(viewContainer, 'click', '.recurring-card', (e, el) => {
    const id = Number(el.dataset.recId);
    const type = el.dataset.recType;
    openRecurringEditor(id, type);
  });
}
...
async function openRecurringEditor(id, type) {
    ...
    <div class="sheet">
      ...
      <div class="sheet-body" style="padding:16px">
        <div class="form-group">
          ...
          <input class="form-input" id="rec-desc" type="text" value="${item?.description ?? ''}" placeholder="e.g. Netflix">
        </div>
        <div class="form-group">
          <label class="form-label">Amount (£)</label>
          <input class="form-input" id="rec-amount" type="number" step="0.01" min="0" value="${item?.amount ?? ''}" placeholder="0.00">
        </div>
        ${isExpense ? `
          <div class="form-group">
          ...
          <div class="form-group" style="display:flex;align-items:center;gap:12px">
            <label class="form-label" style="margin:0">Shared with Rich</label>
            <label class="toggle">
              <input type="checkbox" id="rec-shared" ${item?.isShared ? 'checked' : ''}>
        ...
        ` : `
          <div class="form-group">
            ...
            <input class="form-input" id="rec-interval" type="number" value="${item?.intervalDays ?? 30}" min="1">
          </div>
        `}
        <div class="form-group">
          <label class="form-label">Start date</label>
          <input class="form-input" id="rec-start" type="date" value="${item?.startDate ?? today()}">
        </div>
        <div class="form-group">
          <label class="form-label">End date (leave blank for open-ended)</label>
          <input class="form-input" id="rec-end" type="date" value="${(!item?.endDate || item.endDate === '4001-01-01') ? '' : item.endDate}">
        </div>
        <div style="display:flex;gap:8px;padding-bottom:20px">
          ${item ? `<button class="btn btn-danger" id="rec-del">Delete</button>` : ''}
          <button class="btn btn-primary" id="rec-save" style="flex:1">
            ${item ? 'Update' : 'Save'}
          </button>
        </div>
      </div>
  ...
  if (item) {
    overlay.querySelector('#rec-del').onclick = async () => {
      ...
      if (isExpense) await db.recurringExpenses.delete(id);
      else await db.recurringIncome.delete(id);
      overlay.remove();
      renderRecurring();
    };
  }
  ...
  overlay.querySelector('#rec-save').onclick = async () => {
    ...
    if (isExpense) {
      ...
      if (id) await db.recurringExpenses.update(id, data);
      else await db.recurringExpenses.add(data);
    } else {
      ...
      if (id) await db.recurringIncome.update(id, data);
...
async function renderAnalysis() {
  ...
  const days = [];
  let d = cycle.start;
  while (d <= today() && d <= cycle.end) {
    days.push(d);
    d = addDays(d, 1);
  }

  const dailyData = await Promise.all(days.map(async day => {
    const { balance } = await calcRollingBalance(day);
    return { day, balance };
  }));
  ...
  viewContainer.innerHTML = `
    <div class="analysis-screen">
      <div class="screen-header" style="padding-top:52px">
        <div style="width:34px"></div>
        <span class="screen-title">Analysis</span>
        <div style="width:34px"></div>
      </div>
        ...
        ${catRows.map(([cid, total]) => {
          ...
          return `
            <div class="cat-bar-row">
              <span class="cat-bar-label">${cat?.name ?? 'Other'}</span>
              <div class="cat-bar-track">
                <div class="cat-bar-fill" style="width:${pct}%;background:${cat?.colour ?? '#ccc'}"></div>
              </div>
  ...
  const ctx1 = viewContainer.querySelector('#chart-balance').getContext('2d');
  new Chart(ctx1, {
    type: 'line',
    data: {
  ...
  for (let i = 5; i >= 0; i--) {
    ...
    const ms = isoDate(d);
    const me = isoDate(new Date(d.getFullYear(), d.getMonth() + 1, 0));
    const mTxns = await db.transactions
      .where('date').between(ms, me, true, true)
      .filter(t => t.type === 'expense' || t.type === 'distributed_expense')
      .toArray();
    const total = mTxns.reduce((s, t) => s + Math.abs(t.amount), 0);
    months.push({ label: d.toLocaleDateString('en-GB', { month: 'short' }), total });
  }

  const ctx2 = viewContainer.querySelector('#chart-mom').getContext('2d');
  new Chart(ctx2, {
    type: 'bar',
    data: {
      ...
      datasets: [{
        ...
        backgroundColor: months.map((_, i) => i === 5 ? '#1a73e8' : 'rgba(26,115,232,0.35)'),
        borderRadius: 6,
      }],
    },
    options: {
...
async function renderDistributions() {
  ...
  const renderDist = d => {
    ...
    const total = Math.abs(d.totalAmount);
    ...
    return `
      <div class="dist-card" data-dist-id="${d.id}">
        <div class="dist-card-header">
          <span class="dist-card-name">${d.description}</span>
          <span class="dist-card-amount">${fmt(total)}</span>
        </div>
        <div class="dist-card-meta">
          ${cat?.icon ?? ''} ${cat?.name ?? ''} &bull; ${fmtDate(d.startDate)} - ${fmtDate(d.endDate)}
        </div>
        ${!d.isFinished ? `
          <div class="dist-progress-wrap">
            <div class="dist-progress-fill" style="width:${progress}%"></div>
          </div>
        ` : ''}
      </div>
    `;
  ...
  viewContainer.innerHTML = `
    <div class="distributions-screen">
      <div class="screen-header">
        ...
        <button class="icon-btn" id="dist-add-btn">
          <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round">
            <line x1="12" y1="5" x2="12" y2="19"/><line x1="5" y1="12" x2="19" y2="12"/>
          </svg>
        </button>
      ...
      ${active.length > 0 ? `
        <div style="padding:12px 12px 4px;font-size:12px;font-weight:600;color:var(--text-2);text-transform:uppercase;letter-spacing:.5px">Active</div>
        ${active.map(renderDist).join('')}
      ` : ''}

      ${finished.length > 0 ? `
        <div style="padding:12px 12px 4px;font-size:12px;font-weight:600;color:var(--text-2);text-transform:uppercase;letter-spacing:.5px">Completed</div>
        ${finished.slice(0, 20).map(renderDist).join('')}
      ` : ''}

      ${dists.length === 0 ? `
        <div class="empty-state">
          <div class="empty-icon">📅</div>
          <div class="empty-title">No big expenses</div>
          <div class="empty-text">Spread large costs across a date range so they don't distort your daily balance.</div>
          <button class="btn btn-primary" id="dist-empty-add">Add big expense</button>
        </div>
      ` : ''}
    </div>
  `;
  ...
  delegate(viewContainer, 'click', '.dist-card', (e, el) => {
    openDistEditor(Number(el.dataset.distId));
  });
}
...
async function openDistEditor(id) {
  ...
  overlay.innerHTML = `
    <div class="sheet">
      <div class="sheet-handle"></div>
      <div class="sheet-header">
        <span class="sheet-title">${dist ? 'Edit' : 'Add'} Big Expense</span>
        <button class="sheet-close" id="dist-close">✕</button>
      </div>
      <div class="sheet-body" style="padding:16px">
        <div class="form-group">
          <label class="form-label">Description</label>
          <input class="form-input" id="dist-desc" type="text" value="${dist?.description ?? ''}" placeholder="e.g. Holiday flights">
        </div>
        <div class="form-group">
          <label class="form-label">Total amount (£)</label>
          <input class="form-input" id="dist-amount" type="number" step="0.01" min="0" value="${dist ? Math.abs(dist.totalAmount) : ''}">
        </div>
        <div class="form-group">
          <label class="form-label">Category</label>
          <select class="form-select" id="dist-cat">
            ${cats.map(c => `<option value="${c.id}" ${dist?.categoryId === c.id ? 'selected' : ''}>${c.icon} ${c.name}</option>`).join('')}
          </select>
        </div>
        <div class="form-group">
          <label class="form-label">Spread from</label>
          <input class="form-input" id="dist-start" type="date" value="${dist?.startDate ?? today()}">
        </div>
        <div class="form-group">
          <label class="form-label">Spread to</label>
          <input class="form-input" id="dist-end" type="date" value="${dist?.endDate ?? today()}">
        </div>
        <div class="form-group" style="display:flex;align-items:center;gap:12px">
          <label class="form-label" style="margin:0">This is income</label>
          <label class="toggle">
            <input type="checkbox" id="dist-income" ${dist?.isIncome ? 'checked' : ''}>
            <span class="toggle-slider"></span>
          </label>
        </div>
  ...
  overlay.querySelector('#dist-save').onclick = async () => {
    ...
    if (!description || isNaN(totalAmount) || !startDate || !endDate) {
    ...
    // Delete old child transactions if editing
    if (id) {
      await db.transactions.where('distributionId').equals(id).delete();
    }
    ...
    let distId;
    if (id) {
      await db.distributions.update(id, distData);
      distId = id;
    } else {
      distId = await db.distributions.add(distData);
    }
    ...
    overlay.remove();
    renderDistributions();
    showToast(id ? 'Updated' : `Created ${children.length} daily entries`);
  };
}
...
async function renderAccounts() {
  ...
  for (const s of snapshots) {
    if (!latestByAccount[s.accountId] || s.date > latestByAccount[s.accountId].date) {
      latestByAccount[s.accountId] = s;
    }
  }
  ...
  for (const acc of accounts) {
    ...
    if (!snap) continue;
    if (acc.isAsset) totalAssets += snap.balance;
    else totalLiabilities += Math.abs(snap.balance);
  }
  const netWealth = totalAssets - totalLiabilities;

  const accountIcon = type => {
    const icons = { bank: '🏦', credit: '💳', savings: '🏛️', investment: '📈', loan: '📋', pension: '👴', property: '🏠', mortgage: '🏠', holding: '🤝' };
    return icons[type] ?? '💰';
  };
    ...
    <div class="accounts-screen">
      ...
      <div class="accounts-list" style="padding-bottom:80px">
        <div style="display:flex;justify-content:flex-end;margin-bottom:8px">
          <button class="btn btn-primary" id="snap-add-btn" style="font-size:13px;padding:8px 16px">
            Record snapshot
          </button>
        </div>
        ...
        ${accounts.map(acc => {
          ...
          return `
            <div class="account-card" data-acc-id="${acc.id}">
              <div class="account-icon" style="background:${acc.isAsset ? '#e8f5e9' : '#ffebee'}">
                ${accountIcon(acc.type)}
              </div>
              <div class="account-info">
                <div class="account-name">${acc.name}</div>
                <div class="account-type">${snap ? fmtDate(snap.date) : 'No data'}</div>
              </div>
              <div class="account-balance ${bal != null && bal < 0 ? 'negative' : ''}">
                ${bal != null ? fmt(bal) : '-'}
              </div>
            </div>
          `;
...
async function openSnapshotEntry(accounts, latest) {
  ...
  overlay.innerHTML = `
    <div class="sheet">
      <div class="sheet-handle"></div>
      <div class="sheet-header">
        <span class="sheet-title">Record Balances</span>
        <button class="sheet-close" id="snap-close">✕</button>
      </div>
      <div class="sheet-body" style="padding:16px">
        <div class="form-group">
          <label class="form-label">Date</label>
          <input class="form-input" id="snap-date" type="date" value="${today()}">
        </div>
        ${accounts.map(acc => {
          const prev = latest[acc.id]?.balance;
          return `
            <div class="form-group">
              <label class="form-label">${acc.name}${prev != null ? ` (was ${fmt(prev)})` : ''}</label>
              <input class="form-input snap-val" type="number" step="0.01"
                     data-acc-id="${acc.id}" value="${prev ?? ''}" placeholder="0.00">
            </div>
          `;
        }).join('')}
  ...
  overlay.querySelector('#snap-save').onclick = async () => {
    ...
    overlay.querySelectorAll('.snap-val').forEach(input => {
      ...
      if (val !== '') {
        entries.push({ accountId: Number(input.dataset.accId), date, balance: parseFloat(val), note: '' });
      }
    });
    ...
    overlay.remove();
    renderAccounts();
    showToast(`Saved ${entries.length} balances`);
  };
}
...
async function renderSettings() {
  ...
  viewContainer.innerHTML = `
    <div class="settings-screen">
      ...
      <div class="settings-section">
        ...
        <div class="settings-card">
          <div class="settings-row">
            ...
            <span class="settings-row-label">Monthly savings target</span>
            <span style="color:var(--text-2);font-size:14px">£</span>
            <input type="number" id="setting-savings" value="${savings}" step="50" min="0"
                   style="border:none;outline:none;font-size:15px;text-align:right;width:80px;background:none">
          </div>
          <div class="settings-row">
            <span class="settings-row-icon">📅</span>
            <span class="settings-row-label">Cycle starts on day</span>
            <input type="number" id="setting-cycle" value="${cycleStart}" min="1" max="28"
                   style="border:none;outline:none;font-size:15px;text-align:right;width:50px;background:none">
          </div>
        </div>
      </div>
      ...
      <div class="settings-section">
        ...
        <div class="settings-card">
          <div class="settings-row" id="nav-recurring">
            <span class="settings-row-icon">🔄</span>
            <span class="settings-row-label">Recurring expenses</span>
            <span class="settings-row-chevron">›</span>
          </div>
          <div class="settings-row" id="nav-distributions">
            <span class="settings-row-icon">📅</span>
            <span class="settings-row-label">Big Expenses</span>
            <span class="settings-row-chevron">›</span>
          </div>
          <div class="settings-row" id="nav-accounts">
            <span class="settings-row-icon">🏦</span>
            <span class="settings-row-label">Account snapshots</span>
            <span class="settings-row-chevron">›</span>
          </div>
        </div>
      </div>
      ...
      <div class="settings-section">
        ...
        <div class="settings-card">
          <div class="settings-row" id="nav-import">
            <span class="settings-row-icon">📥</span>
          ...
          <div class="settings-row" id="export-btn">
            <span class="settings-row-icon">📤</span>
            <span class="settings-row-label">Export data (JSON)</span>
            <span class="settings-row-chevron">›</span>
          </div>
          <div class="settings-row" id="clear-btn" style="color:var(--red)">
            <span class="settings-row-icon">🗑️</span>
            <span class="settings-row-label" style="color:var(--red)">Clear all data</span>
          </div>
        </div>
      </div>

      <div style="text-align:center;padding:20px;color:var(--text-2);font-size:12px">
        Pocket Ledger - Personal Finance<br>Data stored locally on this device
      </div>
    </div>
  `;

  // Save settings on change
  viewContainer.querySelector('#setting-savings').onchange = async e => {
    await setSetting('savingsAmount', Number(e.target.value));
    showToast('Savings target updated');
  };

  viewContainer.querySelector('#setting-cycle').onchange = async e => {
    await setSetting('cycleStartDay', Number(e.target.value));
    showToast('Cycle start day updated');
  };
  ...
  viewContainer.querySelector('#clear-btn').onclick = async () => {
    ...
    await Promise.all([
      db.transactions.clear(), db.distributions.clear(),
      db.accountSnapshots.clear(), db.friendTransactions.clear(),
    ]);
    ...
    <div class="import-screen">
      ...
      <div style="padding:16px">
        <p style="color:var(--text-2);font-size:14px;line-height:1.6;margin-bottom:16px">
          Import a <strong>migration-data.json</strong> file generated by the migration script.
          This will add data without overwriting existing records.
        </p>
      ...
      <div style="padding:0 16px">
        <div style="background:var(--card);border-radius:var(--radius);padding:14px;font-size:13px;color:var(--text-2);line-height:1.7">
          ...
          1. Download the repo and locate <code>finance/migrate/extract.py</code><br>
...
async function handleImportFile(file) {
  ...
  try {
    ...
    await step('Importing categories...', 10, async () => {
    ...
    await step('Importing transactions...', 30, async () => {
      if (data.transactions?.length) {
        // Batch in chunks of 500 to avoid memory issues
        for (let i = 0; i < data.transactions.length; i += 500) {
          await db.transactions.bulkPut(data.transactions.slice(i, i + 500));
          progLabel.textContent = `Importing transactions... ${i + 500}/${data.transactions.length}`;
        }
        stats.transactions = data.transactions.length;
      }
    });

    await step('Importing recurring expenses...', 50, async () => {
      if (data.recurringExpenses?.length) {
        await db.recurringExpenses.bulkPut(data.recurringExpenses);
        stats.recurringExpenses = data.recurringExpenses.length;
      }
    });

    await step('Importing recurring income...', 60, async () => {
      if (data.recurringIncome?.length) {
        await db.recurringIncome.bulkPut(data.recurringIncome);
        stats.recurringIncome = data.recurringIncome.length;
      }
    });

    await step('Importing savings targets...', 65, async () => {
      if (data.savingsTargets?.length) {
        await db.savingsTargets.bulkPut(data.savingsTargets);
        stats.savingsTargets = data.savingsTargets.length;
      }
    });

    await step('Importing distributions...', 70, async () => {
      if (data.distributions?.length) {
        await db.distributions.bulkPut(data.distributions);
        stats.distributions = data.distributions.length;
      }
    });

    await step('Importing account snapshots...', 80, async () => {
      if (data.accountSnapshots?.length) {
        await db.accountSnapshots.bulkPut(data.accountSnapshots);
        stats.accountSnapshots = data.accountSnapshots.length;
      }
    });

    await step('Importing friend holdings...', 90, async () => {
      if (data.friendHoldings?.length) {
        await db.friendHoldings.bulkPut(data.friendHoldings);
        stats.friendHoldings = data.friendHoldings.length;
      }
      if (data.friendTransactions?.length) {
        await db.friendTransactions.bulkPut(data.friendTransactions);
        stats.friendTransactions = data.friendTransactions.length;
      }
    });
    ...
    resultDiv.innerHTML = `
      ...
      <button class="btn btn-primary btn-full" style="margin-top:12px" onclick="window.app.navigate('balance')">
        Go to Balance
      </button>
    `;
  } catch (err) {
...
async function exportData() {
  ...
  const [transactions, categories, recurringExpenses, recurringIncome, savingsTargets, distributions, accounts, accountSnapshots, friendHoldings, friendTransactions] = await Promise.all([
    ...
    db.savingsTargets.toArray(),
    db.distributions.toArray(),
    db.accounts.toArray(),
    db.accountSnapshots.toArray(),
    db.friendHoldings.toArray(),
    db.friendTransactions.toArray(),
  ]);
  ...
  a.href = url;
  a.download = `pocket-ledger-backup-${today()}.json`;
  a.click();
...
// Expose navigate globally for onclick handlers in innerHTML
window.app = { navigate };
